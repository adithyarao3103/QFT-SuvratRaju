...
\begin{document}
    ...
    \end{equation*}
    since this is the matrix that relates the in-states to the out-states as required by the \(S-\)matrix. (Note that this is not the \textit{actual} \(S-\)matrix, and in the naive perturbation theory, this can be called \(S\) since our in and out states we start with are from the same Hilbert space, and it is this matrix we use to evolve the in-states in time to obtain the overlap with out states. The actual \(S-\)matrix would be simply the overlap between the in-Hilbert space states and the out-Hilbert space states.) The leading order term in this is the identity. This term simply says that the initial two particles go straight through each other without nothing happening. Therefore, the convention is to calculate \(S-1\). (In some cases, in some condensed matter systems etc, it turns out that the \(1\) also gets corrected when we compute the exact \(S-\)matrices and we obtain some interesting phases.)\\
...
Remember that the difference between one time ordering and another time ordering (i.e. \(\phi(x)\phi(y)\) and \(\phi(y)\phi(x)\)) is just the commutator, and it vanishes as long as the points are spacelike separated, and therefore approximately gives a delta function in \(x-y\). Therefore, when we take the box operator into the time ordering, we will get some terms where the box operator is acting on the fields, and the other terms will form commutators of the above kind, giving \(\delta^4(x_i - x_j)\). These delta functions will end up contracting the \(f_i\)s and \(f_j\)s, and in the limit where the \(f\)s are orthogonal to each other (i.e. they do not overlap), these terms will give \(0\). And therefore, in this limit, it is fine to move the derivatives into the time ordering. \\
...
It is common in field theories that as we go to diagrams with loops, they diverge. The reason is not that we were wrong in our definitions, but the infinities are intrinsic to the theory. Suppose there was an ultimate UV complete field theory that is the actual description at the microscopic scale, then the field theory we are discussing right now is some \textit{effective field theory} of the underlying ultimate field theory. (This statement is true in almost every physical theories, we do not model the flow of a liquid based on the movements of the individual particles in it, and similarly in the case of field theories). At long distances, the microscopic parameters combine to give us some physical observables, and the effective field theory relates one set of physical observables to another set of physical observables. Now if we try to directly relate the physical observables we have to the microscopic parameters, then the relationship will be complicated, giving infinities. Therefore the infinities arise if we not careful about relating physical observables, and if we are asking the wrong physical questions. In our case, we have written down a field theory, and we are not careful with what the limits of the validity of our field theory are. 
